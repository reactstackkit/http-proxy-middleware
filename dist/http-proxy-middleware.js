    ...
    constructor(context, opts) {
        ...
        this.prepareProxyRequest = req => {
            ...
            if (this.proxyOptions.logLevel === 'debug') {
                var arrow = logger_1.getArrow(originalPath, req.url, this.proxyOptions.target, newProxyOptions.target);
                this.logger.debug('[HPM] %s %s %s %s', req.method, originalPath, arrow, newProxyOptions.target);
            }
            return newProxyOptions;
        };
        // Modify option.target when router present.
        this.__applyRouter = (req, options) => {
            var newTarget;
            if (options.router) {
        ...
        // rewrite path
        this.__applyPathRewrite = (req, pathRewriter) => {
            if (pathRewriter) {
